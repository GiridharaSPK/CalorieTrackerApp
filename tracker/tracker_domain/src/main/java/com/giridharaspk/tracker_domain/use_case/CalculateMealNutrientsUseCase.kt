...
class CalculateMealNutrientsUseCase(
    ...
    operator fun invoke(trackedFoods: List<TrackedFood>): Result {
        ...
        val carbsGoal = (caloryGoal * userInfo.carbRatio / caloriesPerCarb).roundToInt()
        val proteinGoal = (caloryGoal * userInfo.proteinRatio / caloriesPerProtein).roundToInt()
        val fatGoal = (caloryGoal * userInfo.fatRatio / caloriesPerFat).roundToInt()
